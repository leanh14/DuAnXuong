...
<form:form modelAttribute="ghe" action="/ghe/add" method="post" class="form-horizontal" enctype="multipart/form-data">
        ...
        <div class="col-sm-10">
            <form:select path="loaiGhe">
                <c:forEach items="${loaighe}" var="lg">
                    <form:option value="${lg.id}">${lg.tenloai}</form:option>
                </c:forEach>
            </form:select>
        </div>
    </div>
        ...
        <div class="col-sm-10">
            <form:select path="phong">
                <c:forEach items="${phong}" var="ph">
                    <form:option value="${ph.id}">${ph.ten}</form:option>
                </c:forEach>
            </form:select>
        </div>
    </div>
